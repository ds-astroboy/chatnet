...
import InputTokenAmtDialog from "../views/dialogs/InputTokenAmtDialog";
import BarrierCheckDialog from "../views/dialogs/BarrierCheckDialog";
import CommonErrorDialog from "../views/dialogs/CommonErrorDialog";
import NftCheckDialog from "../views/dialogs/NftCheckDialog";
import TradeRequestDialog from "../views/dialogs/TradeRequestDialog";
import TradingDialog from "../views/dialogs/TradingDialog";
import SelectItemDialog from "../views/dialogs/SelectItemDialog";
import { MatrixContextsProvider } from "../../contexts";
import WalletContainer from "./WalletContainer";
import TradingResultDialog from "../views/dialogs/TradingResultDialog";
import UserTransactionsDialog from "../views/dialogs/UserTransactionsDialog";
import NavBar from "./NavBar";
import HomePage from "./HomePage";
import LeftPanel from "./LeftPanel";
import DomainNameDialog from "../views/dialogs/DomainNameDialog";
import SolTransactionFailedDialog from "../views/dialogs/SolTransactionFailedDialog";
...
import CongratulationDialog from "../views/dialogs/CongratulationDialog";
import LearnVideoDialog from "../views/dialogs/LearnVideoDialog";
import CreateDaoDialog from "../views/dialogs/CreateDaoDialog";
import NewProposalDialog from "../views/dialogs/NewProposalDialog";
import DaoMembersDialog from "../views/dialogs/DaoMembersDialog";
import NewTreasuryFormDialog from "../views/dialogs/NewTreasuryFormDialog";
import AllTreasuryAccountDialog from "../views/dialogs/AllTreasuryAccountDialog";
import Gallery from "../views/dao/gallery/Gallery";
import PageNotFound from "../views/elements/PageNotFound";
import { Connection } from "@solana/web3.js";
import EthWalletProvider from "./EthWalletProvider";
import { Provider as ReduxProvider } from 'react-redux';
import reduxStore from "../../redux/store";
import NotificationProvider from "./NotificationProvider";
import LatestModalDialog from "../views/dialogs/LatestModalDialog";
import NftCategoryDialog from "../views/dialogs/NftCategoryDialog";
import DomainCategoryDialog from "../views/dialogs/DomainCategoryDialog";
import WalletWarningDialog from "../views/dialogs/WalletWarningDialog";
...
export default class MatrixChat extends React.PureComponent<IProps, IState> {
  ...
  constructor(props, context) {
    ...
    if (this.props.config.sync_timeline_limit) {
      MatrixClientPeg.opts.initialSyncLimit =
        this.props.config.sync_timeline_limit;
    }
    ...
    if (this.screenAfterLogin) {
      ...
      if (
        this.screenAfterLogin.screen.startsWith("room/") &&
        params["signurl"] &&
        params["email"]
      ) {
        // probably a threepid invite - try to store it
        const roomId = this.screenAfterLogin.screen.substring("room/".length);
        ThreepidInviteStore.instance.storeInvite(
          roomId,
          params as unknown as IThreepidInviteWireFormat
        );
      }
    }
    ...
    this.state.resizeNotifier.on(
      "middlePanelResized",
      this.dispatchTimelineResize
    );
    ...
    if (!Lifecycle.isSoftLogout()) {
      Lifecycle.attemptTokenLogin(
        ...
        const firstScreen = this.screenAfterLogin
  ...
  componentWillUnmount() {
    ...
    UIStore.destroy();
    this.state.resizeNotifier.removeListener(
      "middlePanelResized",
      this.dispatchTimelineResize
    );

    if (this.accountPasswordTimer !== null)
  ...
  private setIsShowLeftPanel(isShowLeftPanel) {
  ...
  onAction = (payload) => {
    // console.log("Matrix Chat payload = ", payload);
    ...
    switch (payload.action) {
      case "MatrixActions.accountData":
        ...
        if (payload.event_type === "m.identity_server") {
          const fullUrl = payload.event_content
            ? payload.event_content["base_url"]
            : null;
          if (!fullUrl) {
      ...
      case Action.InputTokenAmt: {
        this.createTokenAmountInputDlg();
        break;
      }
      case "view_create_room":
        this.createRoom(payload.public, payload.defaultName);
        break;
      case "view_barriercheck_dialog":
        this.showBarrierCheckDialog(payload);
        break;
      case "show_nft_check_dialog":
        this.showNftCheckDialog(payload);
        break;
      case "show_nft_category_dialog":
        this.showNftCategoryDialog(payload);
        break;
      case "show_trade_request_dialog":
        this.showTradeRequestDialog(payload);
        break;
      case "waiting_trading_accept":
        this.showTradeRequestDialog(payload);
        break;
      case "show_trading_dialog":
        this.showTradingDialog(payload);
        break;
      case "show_select_item_dialog":
        this.showSelectItemDialog(payload);
        break;
      case "show_trading_result_dialog":
        this.showTradingResultDialog(payload);
        break;
      case "show_Congratulation_Dialog":
        this.showCongratulationDialog(payload);
        break;
      case "show_tokens_nfts_page":
        this.showTokensAndNftsPage();
        break;
      case "show_connect_email_page":
        this.showConnectEmailPage();
        break;
      case "view_common_error_dialog":
        this.showCommonErrorDialog(payload);
        break;
      case "view_create_group": {
        const prototype = SettingsStore.getValue(
      ...
      case "view_home_page":
        ...
        break;
      case "view_terms_page":
        this.setState({currentRoomId: null});
        this.viewTerms();
        break;
      case "view_privacy_page":
        this.setState({currentRoomId: null});
        this.viewPrivacy();
        break;
      case "view_about_page":
        this.setState({currentRoomId: null});
        this.viewAbout();
        break;
      case "view_vote_page":
        ...
        this.viewVote();
        break;
      case "show_dao_suggestion":
        this.showDaoSuggestion(payload);
        break;
      case "show_dao_proposal":
        this.showProposal(payload);
        break;
      case "show_create_dao":
        this.showCreateDaoDialog();
        break;
      case "show_newProposal":
        this.showNewProposal();
        break;
      case "show_dao_members":
        this.showDaoMembers();
        break;
      case "show_newTreasury_form":
        this.showNewTreasuryForm();
        break;
      case "show_AllTreasuryAccount":
        this.showAllTreasuryAccount();
        break;
      case "show_gallery":
        this.showGallery(payload);
        break;
      case "show_domain_name_dialog":
        this.showDomainNameDialog(payload);
        break;
      case "show_domain_category_dialog":
        this.showDomainCategoryDialog(payload);
        break;
      case "show_learn_video":
        this.showLearnVideo();
        break;
      ...
      case "view_create_chat":
        ...
        break;
      case "sol_transaction_failed":
        this.showFailedSolTransactionDialog();
        break;
      case "view_invite":
      ...
      case "reject_cookies":
        ...
        break;
      case "set_webSocket":
        this.setState({ webSocket: payload.webSocket });
        break;
      case "show_latestModal":
        this.showLatestModal(payload);
        break;
      case "show_wallet_warn_dialog":
        this.showWalletWarningModal();
        break;
    }
  };
  ...
  private viewRoom(roomInfo: IRoomInfo) {
    ...
    if (roomInfo.room_alias) {
      ...
      console.log(
        `Switching to room id ${roomInfo.room_id} at event ` + roomInfo.event_id
      );
    }
    ...
    return waitFor.then(() => {
      ...
      const replaceLast =
        presentedId[0] === "#" && roomInfo.room_id === this.state.currentRoomId;
  ...
  private async viewGroup(payload) {
    ...
    if (!this.firstSyncComplete) {
      if (!this.firstSyncPromise) {
        console.warn(
          "Cannot view a group before first sync. group_id:",
          groupId
        );
        return;
      }
  ...
  private viewVote() {
    this.notifyNewScreen("vote");
    this.setPage(PageTypes.VotePage);
  }

  private viewTerms() {
    this.notifyNewScreen("terms");
    this.setPage(PageTypes.TermsPage);
  }
  
  private viewPrivacy() {
    this.notifyNewScreen("privacy");
    this.setPage(PageTypes.PrivacyPage);
  }  

  private viewAbout() {
    this.notifyNewScreen("about");
    this.setPage(PageTypes.AboutPage);
  }  

  private showDaoSuggestion(payload) {
    this.notifyNewScreen("vote/" + payload.symbol);
    this.setState({
      page_type: PageTypes.DaoSuggestion,
      currentVoteRealmSymbol: payload.symbol
    })
  }

  private showProposal(payload) {
    this.notifyNewScreen(`vote/${payload.symbol}/proposal/${payload.proposalPk}`);
    this.setPage(PageTypes.Proposal);
  }
  private showTokensAndNftsPage() {
    this.notifyNewScreen("nfts-and-tokens");
    this.setPage(PageTypes.TokensAndNfts);
  }

  private showConnectEmailPage() {
    this.notifyNewScreen("connect-email");
    this.setPage(PageTypes.ConnectEmail);
  }
  private viewUser(userId: string, subAction: string) {
    // Wait for the first sync so that `getRoom` gives us a room object if it's
    // in the sync response
    const waitForSync = this.firstSyncPromise
      ? this.firstSyncPromise.promise
      : Promise.resolve();
    waitForSync.then(() => {
      if (subAction === "chat") {
  ...
  private async createTokenAmountInputDlg() {
    ...
    //     InputTokenAmtDialog,
    // );
    // const [proceed, amount] = await modal.finished;
  ...
  private async createRoom(defaultPublic = false, defaultName?: string) {
    ...
    //room_uri: room avatar when create new room
    const [shouldCreate, opts, room_uri, barrierObj, category] =
      await modal.finished;
    if (shouldCreate) {
      createRoom(opts, room_uri, barrierObj, category);
    }
  }

  private async showBarrierCheckDialog(payload: any) {
    Modal.createTrackedDialog(
      "BarrierCheck Dialog",
      "",
      BarrierCheckDialog,
      {
        ...
        joinClick: payload.joinClick,
        setIsLoading: payload.setIsLoading,
        barrierInfo: payload.barrierInfo,
        userId: payload.userId,
        accessToken: payload.accessToken,
        setShowConfirmation: payload.setShowConfirmation,
      }
    );
  }

  private async showNftCheckDialog(payload: any) {
    Modal.createTrackedDialog(
      "Nfts in my wallet",
      "",
      NftCheckDialog,
      {
        wallet: payload.wallet,
        adjustableNftAvatar: payload.adjustableNftAvatar
          ? payload.adjustableNftAvatar
          : null,
        getNftData: payload.getNftData ? payload.getNftData : null,
        searchCollectionAddress: payload.searchCollectionAddress,
        network: payload.network
      }
    );
  }

  private async showNftCategoryDialog(payload: any) {
    Modal.createTrackedDialog(
      "Nfts category Dialog",
      "",
      NftCategoryDialog,
      {
        wallets: payload.wallets,
  ...
  private async showTradeRequestDialog(payload: any) {
    Modal.createTrackedDialog(
      "TradeRequest Dialog",
      "",
      TradeRequestDialog,
      {
        tradeUserId: payload.tradeUserId,
        type: payload.type,
        notification_id: payload.notification_id,
        webSocket: this.state.webSocket,
        webSocketData: this.state.webSocketData,
        tradingWebSocket: payload.tradingWebSocket,
      }
    );
  }

  private async showTradingDialog(payload: any) {
    Modal.createTrackedDialog(
      "Trading Dialog",
      "",
      TradingDialog,
      {
        ...
        webSocket: this.state.webSocket,
        tradingWebSocket: payload.tradingWebSocket,
        isInitializer: payload.isInitializer
      },
      null,
      false,
      true
  ...
  private async showSelectItemDialog(payload: any) {
    Modal.createTrackedDialog(
      "SelectItem Dialog",
      "",
      SelectItemDialog,
      {
        ...
        sendItems: payload.getItemsFromSelectDialog,
        nftsData: payload.nftsData,
        nftsTokenData: payload.nftsTokenData,
      }
    );
  }

  private async showTradingResultDialog(payload: any) {
    Modal.createTrackedDialog(
      "TradingResult Dialog",
      "",
      TradingResultDialog,
      {
        type: payload.type,
  ...
  private async showCommonErrorDialog(payload) {
     Modal.createTrackedDialog(
      "CommonError Dialog",
      "",
      CommonErrorDialog,
  ...
  private async showCongratulationDialog(payload) {
    Modal.createTrackedDialog(
  ...
  private async showDomainNameDialog(payload) {
    const modal = Modal.createTrackedDialog(
      "DomainName Dialog",
      "",
      DomainNameDialog,
      {
        wallet: payload.wallet,
        domainProvider: payload.domainProvider
      }
    );
    const [proceed, domainName] = await modal.finished;
    if (proceed) {
      payload.saveDomainName(domainName);
    }
  }

  private async showDomainCategoryDialog(payload) {
    Modal.createTrackedDialog(
      "DomainName Category Dialog",
      "",
      DomainCategoryDialog,
      {
        wallets: payload.wallets,
        saveDomainName: payload.saveDomainName
      }
    );
    
  }
  
  private async showCreateDaoDialog() {
    Modal.createTrackedDialog(
      "Create DAO Dialog",
      "",
      CreateDaoDialog,
  ...
  private async showNewTreasuryForm() {
    Modal.createTrackedDialog(
      "New Treasury Form",
      "",
      NewTreasuryFormDialog
    )
  }

  private async showAllTreasuryAccount() {
    Modal.createTrackedDialog(
      "All Treasury Account",
      "",
      AllTreasuryAccountDialog
    )
  }

  private async showGallery(payload) {
    Modal.createTrackedDialog(
      "All Treasury Account",
      "",
      Gallery,
      {
        governancePk: payload.governancePk
      }
    )
  }

  private async showDaoMembers() {
    Modal.createTrackedDialog(
      "New Proposal",
      "",
      DaoMembersDialog,
  ...
  private async showFailedSolTransactionDialog() {
    Modal.createTrackedDialog(
      "Sol Transaction Failed Dialog",
      "", 
      SolTransactionFailedDialog
    );
  }

  private async showLearnVideo() {
    Modal.createTrackedDialog(
      "Learn Video", 
      "", 
      LearnVideoDialog
  ...
  private async showLatestModal(payload) {
    Modal.createTrackedDialog(
      "latest Modal", 
      "", 
      LatestModalDialog,
  ...
  private async showWalletWarningModal() {
    Modal.createTrackedDialog(
      "Walelt warning Modal", 
      "", 
      WalletWarningDialog,
      {
      },
  ...
  private leaveRoomWarnings(roomId: string) {
    ...
    const joinRules = roomToLeave.currentState.getStateEvents(
      "m.room.join_rules",
      ""
    );
    if (joinRules) {
  ...
  private leaveRoom(roomId: string) {
    ...
    const isSpace = SpaceStore.spacesEnabled && roomToLeave?.isSpaceRoom();
    Modal.createTrackedDialog(
      isSpace ? "Leave space" : "Leave room",
      "",
      QuestionDialog,
      {
  ...
  private forgetRoom(roomId: string) {
    ...
    MatrixClientPeg.get()
      ...
      .then(() => {
        ...
        RoomListStore.instance.manualRoomUpdate(
          room,
  ...
  private async onLoggedIn() {
    ...
    if (this.screenAfterLogin && this.screenAfterLogin.screen) {
      ...
      if (
        ...
        const params =
          ThreepidInviteStore.instance.translateToWireFormat(threepidInvite);
    ...
    await sleep(30);
    if (
      SettingsStore.getValue("showCookieBar") &&
  ...
  showScreen(screen: string, params?: { [key: string]: any }) {
    ...
    if (screen === "register") {
    ...
    } else if (screen.indexOf("vote/") === 0 && screen.indexOf("proposal") !== -1) {
      const routerUrls = location.href.split("/");
      let symbol = routerUrls[5];
      let proposalPk = routerUrls[7];      
      dis.dispatch({
        action: "show_dao_proposal",
        symbol,
        proposalPk
      });
    } else if (screen.indexOf("vote/") === 0) {
      const symbol = screen.substring(5);
      dis.dispatch({
      ...
      dis.dispatch({
        action: "view_terms_page",
      });
    } else if (screen === "privacy") {
      dis.dispatch({
        action: "view_privacy_page",
      });
    } else if (screen === "about") {
      dis.dispatch({
        action: "view_about_page",
      });
    } else if (screen === "nfts-and-tokens") {
      dis.dispatch({
        action: "show_tokens_nfts_page"
      })
    } else if (screen === "connect-email") {
      dis.dispatch({
        action: "show_connect_email_page"
      })
    } else {
  ...
  getFragmentAfterLogin() {
    ...
    if (
      ...
      !["welcome", "login", "register", "start_sso", "start_cas"].includes(
  ...
  render() {
    ...
    if (this.state.view === Views.LOADING) {
      ...
      const isStoreError =
        ...
        this.state.syncError instanceof InvalidStoreError;
      ...
      if (this.state.ready && this.state.page_type && !isStoreError) {
        ...
        view = (
          <LoggedInView
            ...
            currentVoteRealmSymbol={this.state.currentVoteRealmSymbol}
            saveWalletInfo={this.saveWalletInfo}
          />
        );
      } else {
        ...
        if (this.state.syncError && !isStoreError) {
          errorBox = (
            <div className="mx_MatrixChat_syncError">
      ...
      view = (
        <Welcome>
          <div className="mx_MatrixChat_wrapper" id="mx_MatrixChat_wrapper">
            <div className="mx_MatrixChat">
              <LeftPanel
                isMinimized={this.state.collapseLhs || false}
      ...
      view = (
        <div className="mx_MatrixChat_wrapper" id="mx_MatrixChat_wrapper">
          <div className="mx_SignUpPage">
            {/* <NavBar onLoginClick={this.onLoginClick} isSignout={true} /> */}
            <Registration
      ...
      this.state.view === Views.FORGOT_PASSWORD &&
      SettingsStore.getValue(UIFeature.PasswordReset)
    ) {
      view = (
        <div className="mx_MatrixChat_wrapper" id="mx_MatrixChat_wrapper">
          <div className="mx_ForgotPasswordPage">
            {/* <NavBar isSignout={true} /> */}
            <ForgotPassword
      ...
      view = (
        <div className="mx_MatrixChat_wrapper" id="mx_MatrixChat_wrapper">
          <div className="mx_LoginPage">
            <div className="mx_MobileAppAlert d-none">Chat experience is best on desktop. Mobile app coming soon!</div>
            {/* <NavBar isSignout={true} /> */}
            <Login
              ...
              defaultDeviceDisplayName={this.props.defaultDeviceDisplayName}
              onForgotPasswordClick={
                showPasswordReset ? this.onForgotPasswordClick : undefined
              }
              onServerConfigChange={this.onServerConfigChange}
              fragmentAfterLogin={fragmentAfterLogin}
              defaultUsername={
                this.props.startingFragmentQueryParams.defaultUsername as string
              }
              {...this.getServerProperties()}
            />
          </div>
    ...
    const className = classNames("mx_Matrix_wrap", {
      mx_Matrix_welcome_wrap: this.state.view === Views.WELCOME,
    });
    return (
      <ErrorBoundary>
                ...
                <WalletContainer>
                  <EthWalletProvider>
                    <MatrixContextsProvider>
                      <ContextProvider setIsLoading={this.setIsLoading}>
                        <div
                          id="mx_Matrix_wrap"
                          className={className}
